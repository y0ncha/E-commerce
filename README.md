...
**Design Choice (Resilience)**: By moving failed messages here (without retrying deserialization errors, so the consumer does not block), I prevent the main orders partition from stalling. Each message includes metadata headers to facilitate forensic analysis and eventual manual recovery, ensuring zero data loss.
...
**Retries & Delivery Timeouts**:

- **Mechanism**: High retry count with exponential backoff within a bounded delivery window. The API send timeout matches that window.
- **Justification**: Retries absorb transient broker or network issues; when the window is exceeded, the API returns failure so the client gets a timely response and "ghost success" (order appears after a reported failure) is avoided.
...
- **Mechanism**: Session and request timeouts are kept short so that dead or stuck consumers are detected quickly and their partitions can be reassigned. The maximum time between two poll calls is bounded so that slow processing does not cause Kafka to assume the consumer has died and trigger unnecessary rebalances.
- **Justification**: If the consumer takes too long between polls, the broker will treat it as failed and rebalance the group, which can cause duplicate processing and lag. Short timeouts allow fast recovery when a consumer actually fails, while keeping processing lightweight so normal messages complete within the allowed window.
...
- **Mechanism**: Messages that fail deserialization or processing (e.g. malformed JSON, validation failures) are routed to `orders-dlt` so the consumer does not block. A custom error handler sends failed records to `orders-dlt` with metadata headers (`original-topic`, `original-partition`, `original-offset`, `error-reason`, `failed-at`) for forensic analysis and manual recovery. For deserialization failures we do not retry (malformed data will not become valid); the offset is advanced after DLT send to prevent infinite loops.
- **Justification**: Without this mechanism, a single poison pill would block the entire consumer pipeline. Routing to a separate topic allows the consumer to continue processing valid messages while failed ones are stored for investigation and possible redrive.

**Graceful Listener Degradation**:

- **Mechanism**: Readiness probes reflect listener state (e.g. DEGRADED when Kafka listeners are not running), so load balancers can avoid routing traffic to instances that cannot process messages. Health checks expose Kafka connectivity and whether listeners are active.
- **Justification**: Exposing listener state allows orchestration to take unhealthy instances out of rotation, reducing unnecessary retries and resource exhaustion during dependency failures.
...
- **Mechanism**: Retry logic with exponential backoff is implemented at both the producer and consumer levels for broker connections. Connection and timeout settings are tuned for faster failure detection and reconnection.
...
Java Spring Boot applications experience initial startup delays during the first deployment and might take a few seconds to stabilize.
...
Messages routed to `orders-dlt` include enriched metadata headers added by the custom DLT producer:

- `original-topic`: Source topic for message origin tracking
- `original-partition`: Original partition number for sequencing verification
- `original-offset`: Original offset for message recovery
- `error-reason`: Description of the failure (exception type and message)
- `failed-at`: Timestamp when the message was sent to the DLT (audit trail)
