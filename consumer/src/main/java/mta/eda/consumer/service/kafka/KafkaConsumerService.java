...
public class KafkaConsumerService {
     ...
     *
     * @param payload the JSON order message
     * @param orderId the message key (extracted from Kafka header) - used for partitioning
     * @param offset the offset in the partition - used for idempotency tracking
    ...
    public void consumeOrder(
        ...
        logger.info("Received order message: orderId={}, offset={}, payload={}", orderId, offset, payload);
        ...
        try {
            ...
            logger.info("✓ Successfully processed order: orderId={}, status={}", orderId, order.status());

        } catch (IOException e) {
            // JSON deserialization failed - propagate exception
            // The native CommonErrorHandler will retry with exponential backoff
            logger.error("✗ Failed to deserialize order message: {}", e.getMessage(), e);
     ...
     */
    private Order deserializeOrder(String payload) throws IOException {
     ...
     */
    private static class ProcessedMessageInfo {
        final long offset;
        final long processedAt;

        ProcessedMessageInfo(long offset, long processedAt) {
            this.offset = offset;
